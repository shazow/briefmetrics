...
% if c.report.tables.get('summary'):
    <%
        columns = 'ga:pageviews', 'ga:users', 'ga:avgSessionDuration', 'ga:bounceRate', 'ga:sessions'
...
${widgets.render_table(
    c.report.tables['geo'],
    title='Countries',
    report_link=c.report.include_permalinks and h.ga_permalink('report/visitors-geo', c.report.report, date_start=c.report.date_start, date_end=c.report.date_end),
...
${widgets.render_table(
    ...
    title='Devices',
    report_link=c.report.include_permalinks and h.ga_permalink('report/visitors-mobile-overview', c.report.report, date_start=c.report.date_start, date_end=c.report.date_end),
    prefix_links=c.report.base_url,
)}
...
${widgets.render_table(
    ...
    title='Browsers',
    report_link=c.report.include_permalinks and h.ga_permalink('report/visitors-browser', c.report.report, date_start=c.report.date_start, date_end=c.report.date_end),
    prefix_links=c.report.base_url,
)}
